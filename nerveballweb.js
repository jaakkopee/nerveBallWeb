        ...
        for (var j = 0; j < ball_amount; j++) {
            if (i != j) {
                ...
                if (distance < ball_size[i] / 2 + ball_size[j] / 2) {
                    ...
                    ball_x[i] += ball_size[i] / 2 * nbhelper_getX(angle) * 1.25;
                    ball_y[i] += ball_size[i] / 2 * nbhelper_getY(angle) * 1.25;
                    ball_x[j] += ball_size[j] / 2 * nbhelper_getX(angle + Math.PI) * 1.25;
                    ball_y[j] += ball_size[j] / 2 * nbhelper_getY(angle + Math.PI) * 1.25;
...
function displayPoints() {
    ...
    document.getElementById("points").innerHTML = Math.round(totalscore) + "(" + Math.round(player_lastSplitPoints) + ") Balls:" + ball_amount;
}
